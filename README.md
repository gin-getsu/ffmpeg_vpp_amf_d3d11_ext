...
ffmpeg -hwaccel amf -i "input.m2ts" \
  -c:v hevc_amf \
  -vf "vpp_amf=w=1920:h=720:deint=fast:deint_frame_format=tff:reset_sar=true:format=nv12:cx=0:cy=139:cw=1920:ch=802" \
...
ffmpeg -i "input.m2ts" \
  -c:v hevc_amf \
  -vf "format=nv12, vpp_amf=w=1920:h=720:deint=fast:deint_frame_format=tff:reset_sar=true:format=nv12:cx=0:cy=139:cw=1920:ch=802" \
  -c:a copy -f mp4 "output.mp4"
```
